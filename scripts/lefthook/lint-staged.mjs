...
for (const file of stagedFiles) {
  ...
  const list = filesByPackageName.get(packageName) ?? [];
  list.push(absPath);
  filesByPackageName.set(packageName, list);
}

if (filesByPackageName.size === 0) process.exit(0);

let finalStatus = 0;

for (const [packageName, files] of filesByPackageName.entries()) {
  const result = spawnSync(
    "pnpm",
    ["--filter", packageName, "exec", "eslint", ...files],
    { stdio: "inherit" },
  );
